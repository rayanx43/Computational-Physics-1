...
#Potential within a rectangular region: the SOR with charges
...
V[1:-1,1:-1] = 0.00 # naive guess which gives 52 iterations
...
while converge == False:
    ...
    for i in range(1,x-1):
        for j in range(1,y-1):
            #implementing the Successive Over Relaxation (SOR) method with charges
            if i == j:
                R[i,j] = (1-w)*(R[i,j]) + (w/4)*(R[i+1,j]+R[i-1,j]+R[i,j+1]+R[i,j-1]+(Q[i][j]*h**2))
            else:
                R[i,j] = (1-w)*(R[i,j]) + (w/4)*(R[i+1,j]+R[i-1,j]+R[i,j+1]+R[i,j-1]+(Q[i][j]*h**2))    
